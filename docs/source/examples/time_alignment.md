...
With one aligned pulse, fitting emits a warning and falls back to equal clock
rates (``slope = 1``), estimating only the offset. Pulse tables are paired by
ordinal position and truncated to the shorter length, so inspect the pulse
trains and fit quality before applying a model to experimental data.
...
`exact`. If stream times are not monotonically non-decreasing, matching emits a
warning, uses a stable time-sorted view, and maps the matched indices back to the
original input. Clock resets or overlapping time ranges can still make matches
ambiguous.
