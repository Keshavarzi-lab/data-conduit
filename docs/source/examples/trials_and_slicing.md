...
Trial windows include their closing event. With `start_buffer=0`, the next
trial's numeric start equals the preceding trial's closing timestamp, so the
stored bounds are contiguous. The row that closed the preceding trial is still
excluded from the next trial's field-extraction window and belongs only to the
trial it closed.
