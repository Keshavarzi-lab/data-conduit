...
def construct_data_array(
    ...
    if da_attrs is None:
        ...
        da_attrs = {"description": f"Base DataArray for unified coordinates of type {name}", "source": "Constructed using construct_base_da function"}
    ...
    if values is None and test_values is True:
        values = _generate_test_values(times=times, global_coords=global_coords, virtual_map=virtual_map, virtual_coord_names=virtual_coord_names, dict_of=dict_of, verbose=verbose)
    ...
    if dict_of == "dicts":
        for vcoord in virtual_coord_names:
            try:
                vcoord_values = [virtual_map[global_coord][vcoord] for global_coord in global_coords]
    ...
    # iii) Virtual Coordinates (dict-of-tuples/ Positional Fields)
        ...
        for idx, vcoord in enumerate(virtual_coord_names):
            try:
                vcoord_values = [virtual_map[global_coord][idx] for global_coord in global_coords]
    ...
    # ==== Create DataArray
    data_array = xr.DataArray(data=values, dims=["Time", global_coord_name], coords=coords, name=name, attrs=da_attrs)
    if verbose:
...
def construct_lookup_array(
        ...
        for gcoord in global_coords:
            ...
            if dict_of == "tuples":
                if not isinstance(vcoord_entry, (tuple, list)):  # noqa
                    raise TypeError(f'Expected virtual_map to be dict-of-tuples/lists when dict_of="tuples",but got {type(vcoord_entry)} for global coordinate {gcoord}')
                ...
                if not isinstance(vcoord_entry, Mapping):
                    raise TypeError(f'Expected virtual_map to be dict-of-dicts when dict_of="dicts",but got {type(vcoord_entry)} for global coordinate {gcoord}')
    ...
    lookup_array = xr.DataArray(data=np.ones(len(global_coords), dtype=bool), dims=[global_coord_name], coords=coords, name=name, attrs=lookup_attrs)
    if verbose:
...
def update_data_array(
    data_array: xr.DataArray = None, virtual_map: dict = None, dict_of: str = "dicts", dfs_dict: dict = None, fill_value: any = np.nan, verbose: bool = False
) -> xr.DataArray:
    """
