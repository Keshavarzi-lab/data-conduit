...
from .virtual_arrays_core import construct_lookup_array
...
def _register_ulookup_accessor(accessor):
    """Register once globally and reuse whichever package namespace got there first."""
    existing = xr.DataArray.__dict__.get("ulookup")
    if existing is not None:
        return existing._accessor
    return xr.register_dataarray_accessor("ulookup")(accessor)


@_register_ulookup_accessor
class LookupAccessorConstructor:
    ...
    def __init__(self, data_array: xr.DataArray):
        ...
        lookup_array = self.lookup_constructor(lookup_array=None, global_coord_name=None, dict_of=None, verbose=False)
        self.lookup_array = lookup_array

    def lookup_constructor(self, lookup_array: xr.DataArray = None, global_coord_name: str = "global_coord", dict_of: str = "dicts", verbose: bool = False):
        ...
        if not hasattr(self, "lookup_array"):
            ...
            if dict_of == "tuples":
                virtual_map = dict(zip(global_values, zip(*(self._da.coords[name].values.tolist() for name in virtual_coord_names), strict=True), strict=True))

            elif dict_of == "dicts":
                virtual_map = {gcoord: {vcoord: self._da.sel({global_coord_name: gcoord})[vcoord].item() for vcoord in virtual_coord_names} for gcoord in global_values}
            if verbose:
    ...
    def __call__(self, verbose: bool = False, **selectors):
        """
        ...
        """
        global_coord_name = [dim for dim in self._da.dims if dim.lower() != "time"][0]
        return ulookup(lookup_array=self.lookup_array, global_coord_name=global_coord_name, verbose=verbose, **selectors)

    def sel(self, verbose: bool = False, **selectors):
        """Alias for select()."""
        return self.select(verbose=verbose, **selectors)

    def select(self, verbose: bool = False, **selectors):
        """
