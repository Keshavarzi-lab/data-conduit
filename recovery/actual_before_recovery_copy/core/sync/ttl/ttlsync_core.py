...
def build_pulse_table(
    ...
    *,
    ...
    ------
    ValueError
            If either edge array is not one-dimensional or is not sorted in
            ascending order.
    """  # noqa: D206 | Justification: Not really relevant whether indentation uses tabs or spaces in docstring
    rises = np.asarray(rise_times, dtype=float)
    falls = np.asarray(fall_times, dtype=float)

    if rises.ndim != 1 or falls.ndim != 1:
        raise ValueError("rise_times and fall_times must be 1D.")
    if np.any(np.diff(rises) < 0) or np.any(np.diff(falls) < 0):
        raise ValueError("rise_times and fall_times must be sorted in ascending order.")

    # Pair each rise with the next as-yet-unpaired fall at or after it. Advancing
    # one shared fall cursor prevents an orphaned edge from shifting every later
    # pulse while ensuring a fall can belong to at most one rise.
    paired_starts: list[float] = []
    paired_ends: list[float] = []
    fall_index = 0
    for rise in rises:
        fall_index = max(fall_index, int(np.searchsorted(falls, rise, side="left")))
        if fall_index >= len(falls):
            break
        paired_starts.append(float(rise))
        paired_ends.append(float(falls[fall_index]))
        fall_index += 1

    starts = np.asarray(paired_starts, dtype=float)
    ends = np.asarray(paired_ends, dtype=float)
...
def get_ttl_timebase_conversion(
    ...
    *,
    ...
    converted_target_df[f"{use_col}_to_ref"] = model.transform(converted_target_df[use_col].to_numpy(float))
    ...
    if visualisation or error_visualisation_tools:
        from .ttl_visualisation import plot_conversion_error_tools, plot_stacked_pulses
