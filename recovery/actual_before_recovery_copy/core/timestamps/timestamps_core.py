...
def collect_timestamps(
    df: pd.DataFrame | xr.DataArray | None = None, timestamp_name: str | None = None, time_location: str | None = None, verbose: bool = False
) -> pd.Series | None:
    """
    ...
    if isinstance(df, xr.DataArray):  # Type check, convert to DataFrame if it's an xarray DataArray
        # Always provide a collision-free value-column name. ``DataArray.to_dataframe``
        # otherwise rejects unnamed arrays, while ``reset_index`` rejects arrays whose
        # data name is also the timestamp coordinate name (commonly ``Time``).
...
def collect_timestamps_nested(
    ...
    **kwargs,  # l0_selector, l1_selector, l2_selector, etc. for future extensibility
    ...
    # Parse selectors through the shared validator, then check that their depth
    # is valid for this particular nested dictionary.
    ...
    for level_num in level_selectors:
        if level_num >= max_depth:
            raise ValueError(f"Selector 'l{level_num}_selector' is defined for level {level_num}, but the nested dictionary only has a maximum depth of {max_depth}.")
