...
def collect_folders(
    ...
    if not folders:
        raise FileNotFoundError(f"No subfolders found in {base_path}" + (f" matching prefix '{folder_prefix}'" if folder_prefix else ""))
    return folders
    ...
    for item in sorted(base_path.iterdir(), key=lambda p: p.name):
        if item.is_file() and item.match(file_pattern):
            if item.stem in result:
                raise ValueError(f"Directory entries in {base_path} map to duplicate output key {item.stem!r}; file stems and directory names must be unique.")
            ...
            if item.is_dir():
                ...
                if child:  # Only include non-empty branches
                    if item.name in result:
                        raise ValueError(f"Directory entries in {base_path} map to duplicate output key {item.name!r}; file stems and directory names must be unique.")
