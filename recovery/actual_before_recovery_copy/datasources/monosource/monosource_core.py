...
from ...core.utils import _apply_level_selectors, _flatten_nested_dict, _parse_selectors
    ...
    **kwargs
    ...
    if dfs_dict is not None:
        if not isinstance(dfs_dict, dict):
            raise TypeError(f"dfs_dict must be a dict if provided, got {type(dfs_dict).__name__}.")
        return dfs_dict

    if experiment_directory_path is None:
        raise ValueError("Provide either dfs_dict or experiment_directory_path so collect_dfs can build one.")
