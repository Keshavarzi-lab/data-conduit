...
from ...core.io import collect_dfs, read_csv, read_json, split_jsonl, split_yaml
from ...core.utils import starts_with
from .monosource_core import MonoSource, _build_data_arrays
    ...
    **kwargs
    ...
    def __init__(
        ...
        **kwargs
        ...
        if dfs_dict is None and experiment_directory_path is not None:
            readers, mapped_reader_kwargs, self._split_fn = self._resolve_readers(file_type, reader_kwargs)
    ...
    def _resolve_readers(
        ...
        if file_type not in cls._FILETYPE_MAP:
            raise ValueError(f"Unsupported file_type: '{file_type}'. Supported: {list(cls._FILETYPE_MAP.keys())}. For custom formats, pass readers directly to MonoSource.")
    ...
    def _apply_split_to_leaves(
        ...
        for key, value in d.items():
            if isinstance(value, dict):
                result[key] = FileTypeData._apply_split_to_leaves(value, split_fn, verbose)
...
class RotationData(FileTypeData):
    ...
    **kwargs
    ...
    @staticmethod
    def _transform_rotation_leaves(
        d: dict,
        conversion: str | None,
        ...
        for key, value in d.items():
            if isinstance(value, dict):
                result[key] = RotationData._transform_rotation_leaves(value, conversion, angular_range, verbose)
                ...
                if dataframe["Rotation"].isna().any():
                    ...
                    raise ValueError(f"RotationData: 'Rotation' column in '{key}' contains {bad_rows} non-numeric value(s); cannot apply angular transforms.")
                ...
                if conversion == "deg2rad":
                    ...
                    raise ValueError(f"Invalid angular_unit_conversion: '{conversion}'. Must be 'deg2rad' or 'rad2deg'.")

                if angular_range is not None:
                    if not (isinstance(angular_range, list) and len(angular_range) == 2):
                        raise ValueError(f"angular_range must be a list of two floats [min, max]. Got: {angular_range}")
                    min_value, max_value = angular_range
                    dataframe["Rotation"] = ((dataframe["Rotation"] - min_value) % (max_value - min_value)) + min_value
    ...
    **kwargs
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.expected_columns = dict(self.DEFAULT_EXPECTED_COLUMNS) if expected_columns is None else expected_columns
        self.strict = strict
    ...
    def _reconcile_trial_schemas(
        ...
        for key, value in d.items():
            if isinstance(value, dict):
                if "trials" in value and isinstance(value["trials"], pd.DataFrame):
    ...
    def _expand_list_columns(df: pd.DataFrame) -> pd.DataFrame:
        ...
        for col in df.columns:
            ...
            if not non_null.apply(lambda v: isinstance(v, list)).any():
                continue
            max_len = int(df[col].apply(lambda v: len(v) if isinstance(v, list) else 0).max())
            for i in range(max_len):
                new_df[f"{col}_{i}"] = df[col].apply(lambda v, i=i: v[i] if isinstance(v, list) and i < len(v) else None)
