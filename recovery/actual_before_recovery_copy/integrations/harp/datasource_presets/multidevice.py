...
from ....datasources.multisource.multisource_core import MultiSource
from ..harptools import collect_harp_dfs

################################################################################

_DEFAULT_VIRTUAL_COORD_NAMES = ["device", "register", "localID"]
_NOSEPOKE_DEVICE_LIST = [f"Behavior{i}" for i in range(6)]
_NOSEPOKE_DEVICE_REGISTERS = {f"Behavior{i}": ["32", "34"] for i in range(6)}
_NOSEPOKE_VIRTUAL_MAPS = {
    "Activations": {
        f"NP_{i}": {
            "device": f"Behavior{i // 3}",
            "register": "32",
            "localID": ["DIPort0", "DIPort1", "DIPort2"][i % 3],
        }
        for i in range(18)
    },
    "LEDs": {
        f"NP_{i}": {
            "device": f"Behavior{i // 3}",
            "register": "34",
            "localID": ["DOPort0", "DOPort1", "DOPort2"][i % 3],
        }
        for i in range(18)
    },
    "Valves": {
        f"NP_{i}": {
            "device": f"Behavior{i // 3}",
            "register": "34",
            "localID": ["DOPort3", "DOPort4", "DOPort5"][i % 3],
        }
        for i in range(18)
    },
    "Rewards": {
        f"NP_{i}": {
            "device": f"Behavior{i // 3}",
            "register": "32",
            "localID": ["DIPort3", "DIPort4", "DIPort5"][i % 3],
        }
        for i in range(18)
    },
}
_NOSEPOKE_DATA_KEYS = ["Activations", "LEDs", "Valves", "Rewards"]
...
class MultiDevice(MultiSource):
    ...
    **kwargs
    ...
    def __init__(
        ...
        virtual_coord_names: list[str] | None = _DEFAULT_VIRTUAL_COORD_NAMES,
        dict_of: str = "dicts",
        ...
        **kwargs,
        ...
        if virtual_coord_names is _DEFAULT_VIRTUAL_COORD_NAMES:
...
class Nosepoke(MultiDevice):
    ...
    **kwargs
    ...
    def __init__(
        ...
        experiment_directory_path: str | Path | None = None,
        harp_device_yaml_path: str | Path = "./device.yml",
        device_list: list[str] = _NOSEPOKE_DEVICE_LIST,
        device_registers: dict = _NOSEPOKE_DEVICE_REGISTERS,
        virtual_maps: dict = _NOSEPOKE_VIRTUAL_MAPS,
        data_keys: list[str] = _NOSEPOKE_DATA_KEYS,
        global_coord_name: str = "peripherals",
        virtual_coord_names: list[str] = _DEFAULT_VIRTUAL_COORD_NAMES,
        rekey: bool = True,
        verbose: bool = False,
        **kwargs,
    ):
        """Initialise Nosepoke preset and delegate to MultiDevice."""
        if device_list is _NOSEPOKE_DEVICE_LIST:
            device_list = deepcopy(device_list)
        if device_registers is _NOSEPOKE_DEVICE_REGISTERS:
            device_registers = deepcopy(device_registers)
        if virtual_maps is _NOSEPOKE_VIRTUAL_MAPS:
            virtual_maps = deepcopy(virtual_maps)
        if data_keys is _NOSEPOKE_DATA_KEYS:
