...
from ....datasources.monosource.monosource_core import MonoSource
from ..harptools import collect_harp_dfs

################################################################################

_SOUND_CARD_DEVICE_LIST = ["SoundCard"]
_SOUND_CARD_DEVICE_IDS = {"SoundCard": "SoundCard0"}
_SOUND_CARD_REGISTERS = {"SoundCard": ["8", "32", "33", "35"]}
_SOUND_CARD_DATA_ARRAYS = {
...
_CAMERA_DEVICE_LIST = [f"Behavior{i}" for i in range(6)]
_CAMERA_DEVICE_IDS = {f"Behavior{i}": f"ID_{i}" for i in range(6)}
_CAMERA_START_REGISTERS = {f"Behavior{i}": ["78"] for i in range(6)}
_CAMERA_START_DATA_ARRAYS = {
    f"Behavior{i}_StartCameras": {
        "l0_selector": f"Behavior{i}",
        "l1_selector": "78",
    }
    for i in range(6)
}
_CAMERA_FRAME_REGISTERS = {f"Behavior{i}": ["92"] for i in range(6)}
_CAMERA_FRAME_DATA_ARRAYS = {
    f"Behavior{i}_Camera0Frame": {
        "l0_selector": f"Behavior{i}",
        "l1_selector": "92",
    }
    for i in range(6)
...
def _filter_to_matching_times(data_arrays: dict[str, xr.DataArray], verbose: bool) -> dict[str, xr.DataArray]:
    ...
    filtered = {name: data_array.sel(Time=common_times) for name, data_array in data_arrays.items()}
...
class SoundCard(Device):
    ...
    **kwargs
    ...
    def __init__(
        ...
        # == expected device layout ===#
        device_list: list[str] | None = _SOUND_CARD_DEVICE_LIST,
        device_IDs: dict | None = _SOUND_CARD_DEVICE_IDS,
        device_registers: dict | None = _SOUND_CARD_REGISTERS,
        # == named data outputs ===#
        device_data_arrays: dict | None = _SOUND_CARD_DATA_ARRAYS,
        rekey: bool = True,
        # == loader behavior ===#
        verbose: bool = False,
        **kwargs,
        ...
        if device_list is _SOUND_CARD_DEVICE_LIST:
            device_list = deepcopy(device_list)
        if device_IDs is _SOUND_CARD_DEVICE_IDS:
            device_IDs = deepcopy(device_IDs)
        if device_registers is _SOUND_CARD_REGISTERS:
            device_registers = deepcopy(device_registers)
        if device_data_arrays is _SOUND_CARD_DATA_ARRAYS:
...
class CameraStart(Device):
    ...
    **kwargs
    ...
    def __init__(
        ...
        harp_device_yaml_path: str | Path = "./device.yml",
        # == expected device layout ===#
        device_list: list[str] | None = _CAMERA_DEVICE_LIST,
        device_IDs: dict | None = _CAMERA_DEVICE_IDS,
        device_registers: dict | None = _CAMERA_START_REGISTERS,
        # == named data outputs ===#
        device_data_arrays: dict | None = _CAMERA_START_DATA_ARRAYS,
        rekey: bool = True,
        # == loader behavior ===#
        verbose: bool = False,
        **kwargs,
        ...
        if device_list is _CAMERA_DEVICE_LIST:
            device_list = deepcopy(device_list)
        if device_IDs is _CAMERA_DEVICE_IDS:
            device_IDs = deepcopy(device_IDs)
        if device_registers is _CAMERA_START_REGISTERS:
            device_registers = deepcopy(device_registers)
        if device_data_arrays is _CAMERA_START_DATA_ARRAYS:
            device_data_arrays = deepcopy(device_data_arrays)
...
class Camera0Frames(Device):
    ...
    **kwargs
    ...
    def __init__(
        ...
        device_list: list[str] | None = _CAMERA_DEVICE_LIST,
        device_IDs: dict | None = _CAMERA_DEVICE_IDS,
        device_registers: dict | None = _CAMERA_FRAME_REGISTERS,
        # == named data outputs ===#
        device_data_arrays: dict | None = _CAMERA_FRAME_DATA_ARRAYS,
        rekey: bool = True,
        # == timing helper mode ===#
        ...
        **kwargs,
        ...
        if device_IDs is _CAMERA_DEVICE_IDS:
            device_IDs = deepcopy(device_IDs)
        if device_registers is _CAMERA_FRAME_REGISTERS:
            device_registers = deepcopy(device_registers)
        if device_data_arrays is _CAMERA_FRAME_DATA_ARRAYS:
            device_data_arrays = deepcopy(device_data_arrays)
