...
from ....core.io import collect_dfs
from ....core.utils import _matches_selector, starts_with
...
def construct_device_reader(harp_device_yaml_path: str | Path):
    """
    ...
    """
    # harp-python is an optional dependency: import it lazily so that merely
    # importing this module never requires it. Only the code paths that build a
    # real reader pull it in.
    try:
        import harp
    except ImportError as e:
        raise ImportError("HARP support requires harp-python. Install with: pip install data-conduit[harp]") from e

    if not isinstance(harp_device_yaml_path, str | Path):
        raise TypeError(f"harp_device_yaml_path must be a str or Path, got {type(harp_device_yaml_path)}")
...
def collect_registers(harp_device_yaml_path: str | Path, register_addresses: int | str | list[int] | list[str], verbose: bool = False) -> str | list[str] | None:
    """
    Find register name(s) by address(es) in the schema file.
    Efficiently loads the YAML file once for multiple lookups.
...
def read_harp_bin(
    ...
    **kwargs,
    ...
    if register_name is None:
        raise ValueError(f"No register found for address {register_address} (parsed from '{Path(path).name}').")
    ...
    for device_name, value in dfs_dict.items():
        ...
        for key, item in value.items():
            ...
            if len(parts) > 1:
                ...
                if verbose:
                    print(f"Warning: Cannot parse register address from '{key}' for device '{device_name}'. Keeping original key.")
                register_key = key

            if register_key in register_dict:
                if verbose:
                    print(f"Warning: Register key '{register_key}' already exists for device '{device_name}'. Keeping original key '{key}'.")
...
def _filter_device_registers(
    ...
    for device_name, registers in dfs_dict.items():
        ...
        if isinstance(registers, dict):
            filtered[device_name] = {reg_name: item for reg_name, item in registers.items() if reg_name in allowed}
        else:
            filtered[device_name] = registers
...
def collect_harp_dfs(
    ...
    **kwargs
    ...
    # Always retain the device-type boundary while allowing callers to narrow
    # the top-level selection further. Popping the caller's selector also avoids
    # passing ``l0_selector`` twice to ``collect_dfs``.
