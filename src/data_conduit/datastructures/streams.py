...
def attach_level(
    ...
    ValueError
        ...
        variable; or if it matches a DataFrame column or ``DataFrame.index.names``.
    TypeError
    ...
    if isinstance(data, pd.DataFrame):
        ...
        if level_name in data.columns or level_name in data.index.names:
...
def _combine_dataframe_streams(
    ...
    All members must be DataFrames with the same ordered ``DataFrame.index.names``.
    A named index or a MultiIndex with any named level is retained because its
    values may identify per-session time or another selectable coordinate. When
    every level is unnamed, concatenation creates a fresh ``RangeIndex``. Mixing
    different level names or different numbers of levels is rejected.
    ...
    session_coord : str
        ...
        collide with an existing column or ``DataFrame.index.names``.
    ...
    ValueError
        If ``DataFrame.index.names`` differs between members or ``session_coord``
        collides with an existing column or ``DataFrame.index.names``.
    """
    ...
    # Compare every level in order: MultiIndex.name is normally None even when
    # its Time/keypoint levels carry names that must survive combination.

    index_names = {tuple(obj.index.names) for obj in dataframes}
    ...
    if any(
        session_coord in obj.columns or session_coord in obj.index.names
    ...
    preserve_index = any(name is not None for name in dataframes[0].index.names)
...
def _combine_stream_container(
    ...
    *,
    ...
    ``DataFrame.index.names``. A shared named index is preserved because it may
